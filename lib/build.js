(function () {
  ...
  // Parse yaml workshop data
  const file = fse.readFileSync(`${srcPath}/data/workshopData.yml`, 'utf8')
  const workshopData = yaml.parse(file)

  // read timetable.md and convert to HTML
  const file2 = fse.readFileSync(`${srcPath}/data/timeTable.md`, 'utf8')
  const content = fm(file2)
  const timeTable = md.render(content.body)
  ...
  try {
    pages.forEach((file) => {
      ...
      switch (name) {
        ...
        case 'a-kizombarol-es-a-sembarol.ejs':
          layoutContent = ejsRender(fse.readFileSync(`${srcPath}/layouts/subpage.ejs`, 'utf-8'), Object.assign({}, config, postsDataForIndexPage, {
            title: 'A kizombáról és a sembáról | ' + config.site.title,
            body: pageContents,
            canonicalUrl: config.site.url + '/' + fileData.name,
            description: config.site.quote,
            workshopData: workshopData
          }), { filename: `${srcPath}/layouts/subpage.ejs` })
          break
